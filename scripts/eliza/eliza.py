"""ELIZA (Weizenbaum's DOCTOR script) -- vendored, see README.md.

Upstream: https://github.com/wadetb/eliza @ 6055a7d, MIT (LICENSE).

Local modifications, both so the same seed reproduces a session and
nothing else -- the matching, the script and the reassembly are
upstream's:

- `Eliza(rng=...)` takes a random source (default: the `random`
  module), used by `initial`, `final` and the memory pop.
- `load()` opens the script as UTF-8 rather than in the locale
  encoding.
"""
import logging
import random
...
class Eliza:
    ...
    def respond(self, text):
        ...
        if not output:
            if self.memory:
                index = self.rng.randrange(len(self.memory))
    ...
    def initial(self):
        return self.rng.choice(self.initials)
