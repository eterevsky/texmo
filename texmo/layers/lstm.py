...
class LstmJax(LayerJax):
    ...
    Note: unlike nn.LSTM, uses one bias per gate (not two). The JAX
    param count is thus 4*size less than LstmDef.num_weights.
    """
...
class LstmDef(LayerDef):
    ...
    def num_weights(self) -> int:
        # Single-bias per gate (matches LstmJax). Torch's nn.LSTM uses two
        # biases per gate, so its actual param count is 4*size larger.
